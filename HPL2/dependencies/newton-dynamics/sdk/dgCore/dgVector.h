...
#if defined(DG_SCALAR_VECTOR_CLASS) || (!defined(__ANDROID__) && (defined(__aarch64__) || defined(__arm__)))
	// upstream's dgVectorArmNeon.h is an incomplete/bit-rotted backend (missing
	// arm_neon.h, references undefined DG_ARCH_NEON_64 paths) - use the portable
	// scalar backend on non-Android ARM until that's fixed upstream.
